...
get '/books/:id' do
  ...
  unless validate_numeric_id(book_id)
    halt 400, json({ error: 'Invalid ID format. ID must be a positive integer' })
  def parse_json_body
...
post '/books' do
  ...
  if published_year && !published_year.is_a?(Integer)
    halt 400, json({ error: 'Published year must be a number' })
  end
  
  if isbn && isbn.to_s.length > 13
    halt 400, json({ error: 'ISBN must not exceed 13 characters' })
  end
  ...
  begin
    ...
    halt 400, json({ error: 'ISBN already exists' })
  ensure
    conn.close if conn
  end
